...
### At Task closure (Analyst reopens after PO accepts)

First action is Topology-conditional (D1): **in-repo**, sync the local default branch (checkout + pull; delete the merged local task branch; halts if dirty); **no-repo / detached**, no branch to sync or delete — closure is file operations only (halt if the produced files are in an unexpected state). Then the full closure beat per `WAY_OF_WORKING.md § Task closure` — ROADMAP, BACKLOG, archive, archive-path sweep, **promote earned files in `PROJECT_GUIDE.md` § File index**, **surface `qa_review.md` § Post-merge checklist verbatim to PO** (do not run the checks — D3), **run the direction re-ask when the closure completes a Feature, the recorded Intent has changed, or PO asks** (`WAY_OF_WORKING.md § The direction re-ask` — re-read `CANVAS.md` when present, every `Provisional` row whose `Revisit when:` trigger may have fired, every unresolved `Challenged:` row, and the locked design system when the project has one; surface to PO, do not re-decide — D3; then route every answer before the beat closes — a `DECISIONS.md` row per D2, or a `BACKLOG.md` item written during the re-ask), **write `CHANGELOG.md` entry when the closure marks a release boundary** (paired with the tag suggestion), tag suggestion, next-step routing. For spec-less chore Tasks where Analyst was never invoked, PO produces the closure init directly.
