...
**The terminal control is PO's acceptance gate.** *PO reviews the work and accepts it* — and **merging a review request is the in-repo implementation of that gate, not the gate itself.** The gate is universal and keeps its full force in every topology; what varies is only what "accept" mechanically does. QA reviewing a *diff* is an in-repo convenience, never a requirement — the role loop (DEV implements, QA reviews against AC, PO accepts) is invariant.
...
- **In-repo** — Octopus layers and output live in the project repo. Standard D1 above applies in full: QA opens the review request on 100% Pass, PO accepts, and **acceptance merges**. Every mechanic of the bullets above stands unchanged.
- **Detached** — Octopus layers live in a separate personal folder; the *subject* of the work is a different repo the human may or may not have write access to. D1's git operations apply only to the subject repo *if* PO has write access there. PO's own Octopus artifacts persist as disk saves only — no branches, no commits, no review requests on PO's side; **acceptance follows the no-repo shape** (PO reviews the produced files in place and marks the Task accepted). The subject repo's own acceptance, if any, is the external team's, outside Octopus. Agents do not write to the subject repository under any condition unless explicitly authorized.
- **No-repo** — no subject repo at all; state and output live in a folder. No git rules apply (optional local versioning only): there is no branch and no review request, so QA's Pass verdict signals, **PO reviews the produced files in place and accepts, and acceptance marks the Task accepted** and releases closure (the disk-save shape — status update + archive as file operations).

**D2 — Decision authority.**
Cross-cutting decisions (affecting more than the current Task) are surfaced by agents as Open Questions per D9. PO answers; the agent writes the decision row directly to `DECISIONS.md` — slug, value, rationale, date. PO does not write to `DECISIONS.md` directly.

**Immutability is of the record, not of the question.** A written decision is never edited or deleted — the row and its rationale stand as the durable account of what was decided and why. Whether the decision is still *right* stays askable at any time. To change one, supersede it with a new row referencing the old one and move the old row to `## Superseded Decisions` per `DOCUMENT_TEMPLATES.md § DECISIONS.md`. "Locked" binds the work; it does not close the conversation.

**A row may be locked provisionally.** When a choice must bind now but rests on something not yet known, write it with the state **`Provisional`** and a **`Revisit when:`** line naming the specific thing that re-opens it — a number measured, a customer signed, a threshold reached. A provisional row **binds exactly as a locked one does**: work proceeds on it, sessions do not re-litigate it, and it is never a draft to route around. The only difference is that the moment it gets re-examined is written down instead of assumed. Prefer it to a full lock whenever the rationale itself rests on an estimate.

**A challenge anchors in the row it challenges.** Evidence that contradicts a row is recorded **on that row** as a **`Challenged:`** line — date, one line of what contradicts it, and a pointer to the document holding the evidence. It never lives only in the document that produced it: a reader of `DECISIONS.md` must be able to see that a row is contested without having found the other document first.

**Surfacing a supersession is a duty, not a filing procedure.** An agent about to write — into any document — a finding, analysis, or conclusion that contradicts a decision does not file it and move on. It re-examines the decision against the new evidence and surfaces the direction question to PO as an Open Question, with the same weight as any decision-shaped OQ (D9). Whatever PO then decides — supersede it, supersede it with a provisional row carrying a trigger, or let it stand — the challenge is anchored on the row per the paragraph above. A row is never left reading as an unexamined lock while a document elsewhere contradicts it.
